    ...
    if(req.user){
        try{
           ...
           if(req.body.voter === review.user.toString()){
            ...
            // check if the auth has downvoted the review
            if(downvoters.indexOf(req.body.voter) > -1){
                review.downvoters.splice(downvoters.indexOf(req.body.voter));
                review.upvoters.push(req.body.voter);
                review.downvotes -= 1;
                review.upvotes += 1;
                const updatedReview = await review.save();
                if(updatedReview){
                    return res.status(200).json({success: true, message: 'You upvoted this review.', review: updatedReview});
                }
            // toggle upvote if there...
            }else if(upvoters.indexOf(req.body.voter) > -1){
                review.upvoters.splice(upvoters.indexOf(req.body.voter));
                review.upvotes -= 1;
                const toggledVotedReview = await review.save();
                if(toggledVotedReview){
                    return res.status(200).json({success: true, message: 'Upvote dismissed'});
                }
            }else{
                review.upvoters.push(req.body.voter);
                review.upvotes +=1;
                const upvoted = await review.save();
                if(upvoted){
                    return res.status(200).json({success: true, message: 'You upvoted that review'});
                }
            }
           }
